...
def dmplug(model, scheduler, logdir, img='00000', eta=0, lr=1e-2, dataset='celeba',img_model_config=None,task_config=None,device='cuda'):
    ...
    ref_numpy = np.array(ref_img.cpu().squeeze().permute(1,2,0))
    to_img(ref_img, os.path.join(logdir, 'gt.png'))
    ...
    plt.imsave(os.path.join(logdir, 'degraded.png'), clear_color(y_n), cmap='gray')

    # DMPlug
    # 生成初始点，损失函数，优化器
    Z = torch.randn((1, 28, 256, 256), device=device, dtype=dtype, requires_grad=True)
    ...
    for iterator in pbar:
        ...
        with torch.no_grad():
            ...
            # print(rec_img_torch.shape, gt_img_torch.shape)
    ...
    figure_img, index = plt.subplots(1,4, figsize=(10,4))
    for i in range(4):
        index[i].imshow(best_img[:,:,i])
        index[i].axis('off')
        index[i].set_title('band {}'.format(i+1))          
    plt.tight_layout()  
    plt.savefig(os.path.join(logdir, 'rec_img.png'))
    plt.close()
